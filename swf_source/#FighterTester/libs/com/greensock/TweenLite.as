 ...
 *  object over time, updating them on every frame. Sounds simple, but there's a wealth of capabilities and
 * conveniences
 ...
 * 	<ul>
 * 		<li><b> SPEED </b>- TweenLite has been highly optimized for maximum performance. See some speed comparisons
 * yourself at
 * 			 <a
 * href="http://www.greensock.com/tweening-speed-test/">http://www.greensock.com/tweening-speed-test/</a></li>
 *
 * 		<li><b> Feature set </b>- In addition to tweening ANY numeric property of ANY object, TweenLite can tween
 * filters,  hex colors, volume, tint, frames, and even do bezier tweening, plus LOTS more. TweenMax extends  TweenLite
 * and adds even more capabilities like repeat, yoyo, repeatDelay, timeScale, event dispatching, on-the-fly
 * destination value updates, rounding and more. Overwrite management is an important consideration in  a tweening
 * engine as well which is another area where the GreenSock Tweening Platform shines.  You have options for AUTO
 * overwriting or you can manually define how each tween will handle overlapping  tweens of the same object.</li>
 *
 * 		<li><b> Expandability </b>- With its plugin architecture, you can activate as many (or as few) features as
 * your  project requires. Write your own plugin to handle particular special properties in custom ways. Minimize
 * bloat, and maximize performance.</li>
 *
 * 		<li><b> Sequencing, grouping, and management features </b>- TimelineLite and TimelineMax make it
 * surprisingly  simple to create complex sequences or groups of tweens that you can control as a whole. play(),
 * pause(), restart(),  or reverse(). You can even tween a timeline's <code>currentTime</code> or
 * <code>currentProgress</code> property  to fastforward or rewind the entire timeline. Add labels, gotoAndPlay(),
 ...
 *
 * 		<li><b> Updates </b>- Frequent updates and feature additions make the GreenSock Tweening Platform reliable
 * and robust.</li>
 *
 ...
 *
 * <ul>
 * 	<li><b> delay : Number</b>			Amount of delay in seconds (or frames for frames-based tweens) before the
 * tween should begin.</li>
 *
 * 	<li><b> useFrames : Boolean</b>		If useFrames is set to true, the tweens's timing mode will be based on
 * frames.  Otherwise, it will be based on seconds/time. NOTE: a tween's timing mode is  always determined by its
 * parent timeline. </li>
 *
 * 	<li><b> ease : Function</b>			Use any standard easing equation to control the rate of change. For
 * example,
 * 										<code>Elastic.easeOut</code>. The Default is Quad.easeOut.</li>
 *
 * 	<li><b> easeParams : Array</b>		An Array of extra parameters to feed the easing equation. This can be
 * useful when  using an ease like <code>Elastic</code> and want to control extra parameters like the amplitude  and
 * period.	Most easing equations, however, don't require extra parameters so you  won't need to pass in any
 * easeParams.</li>
 *
 * 	<li><b> immediateRender : Boolean</b> Normally when you create a tween, it begins rendering on the very next
 * frame (when  the Flash Player dispatches an ENTER_FRAME event) unless you specify a <code>delay</code>. This  allows
 * you to insert tweens into timelines and perform other actions that may affect  its timing. However, if you prefer to
 * force the tween to render immediately when it is  created, set <code>immediateRender</code> to true. Or to prevent a
 * tween with a duration of zero from rendering immediately, set <code>immediateRender</code> to false.</li>
 *
 *  <li><b> onInit : Function</b>		A function that should be called just before the tween inits (renders for the
 * first time). Since onInit runs before the start/end values are recorded internally, it is a good place to run code
 * that affects the target's initial position or other tween-related properties. onStart, by contrast, runs AFTER the
 * tween inits and the start/end values are recorded internally. onStart is called every time the tween begins which
 * can happen more than once if the tween is restarted multiple times.</li>
 *
 *  <li><b> onInitParams : Array</b>	An Array of parameters to pass the onInit function.</li>
 *
 *  <li><b> onStart : Function</b>		A function that should be called when the tween begins (when its currentTime
 * is at 0 and  changes to some other value which can happen more than once if the tween is restarted multiple
 * times).</li>
 *
 * 	<li><b> onStartParams : Array</b>	An Array of parameters to pass the onStart function.</li>
 *
 * 	<li><b> onUpdate : Function</b>		A function that should be called every time the tween's time/position is
 * updated
 ...
 *
 * 	<li><b> onReverseComplete : Function</b> A function that should be called when the tween has reached its
 * starting point again after having been reversed. </li>
 *
 ...
 *
 * 	<li><b> overwrite : int</b>			Controls how (and if) other tweens of the same target are overwritten by
 * this tween. There are several modes to choose from, but only the first two are available in TweenLite unless
 * 										<code>OverwriteManager.init()</code> has been called (please see
 * 										<a
 * href="http://www.greensock.com/overwritemanager/">http://www.greensock.com/overwritemanager/</a>  for details and a
 * full explanation of the various modes):
 * 										<ul>
 * 			  								<li>NONE (0) (or false) </li>
 *
 * 											<li>ALL_IMMEDIATE (1) (or true) - this is the default mode in
 * TweenLite</li>
 *
 * 											<li>AUTO (2) - this is the default mode if TweenMax, TimelineLite, or
 * TimelineMax is used in the swf. (these classes automatically init() OverwriteManager if you haven't done so
 * already)</li>
 *
 ...
 * <b>EXAMPLES:</b> <br /><br />
 *
 * 	Please see <a href="http://www.tweenlite.com">http://www.tweenlite.com</a> for examples, tutorials, and
 * interactive demos. <br /><br />
 *
 ...
 * 	  <code>TweenLite.to(mc, 2, {x:"-20"});</code> it'll move the mc.x to the left 20 pixels which is the same as
 * doing
 * 	  <code>TweenLite.to(mc, 2, {x:mc.x - 20});</code> You could also cast it like: <code>TweenLite.to(mc, 2,
 * {x:String(myVariable)});</code></li>
 *
 * 	<li> You can change the <code>TweenLite.defaultEase</code> function if you prefer something other than
 * <code>Regular.easeOut</code>.</li>
 *
 ...
 * <code>TweenLite.killDelayedCallsTo(myFunction);</code> This can be helpful if you want to preempt a call.</li>
 *
 * 	<li> Use the <code>TweenLite.from()</code> method to animate things into place. For example, if you have things
 * set up on  the stage in the spot where they should end up, and you just want to animate them into place, you can
 ...
 * </ul>
 *
 * <b>Copyright 2011, GreenSock. All rights reserved.</b> This work is subject to the terms in <a
 * href="http://www.greensock.com/terms_of_use.html">http://www.greensock.com/terms_of_use.html</a> or for corporate
 * Club GreenSock members, the software agreement that was issued with the corporate membership.
 * 
 ...
 */	 
	public class TweenLite extends TweenCore {
		 ...
		 * Constructor
		 *  
		 * @param target Target object whose properties this tween affects. This can be ANY object, not just a
		 *     DisplayObject.
		 * @param duration Duration in seconds (or in frames if the tween's timing mode is frames-based)
		 * @param vars An object containing the end values of the properties you're tweening. For example, to tween to
		 *     x=100, y=100, you could pass {x:100, y:100}. It can also contain special properties like "onComplete",
		 *     "ease", "delay", etc.
		 */
		 ...
		 * 
		 * @param vars An object containing a corresponding property for each one that should be killed. The values
		 *     don't really matter. For example, to kill the x and y property tweens, do
		 *     <code>myTween.killVars({x:true, y:true});</code>
		 * @param permanent If true, the properties specified in the vars object will be permanently disallowed in the
		 *     tween. Typically the only time false might be used is while the tween is in the process of initting and
		 *     a plugin needs to make sure tweens of a particular property (or set of properties) is killed.
		 * @return Boolean value indicating whether or not properties may have changed on the target when any of the
		 *     vars were disabled. For example, when a motionBlur (plugin) is disabled, it swaps out a BitmapData for
		 *     the target and may alter the alpha. We need to know this in order to determine whether or not a new
		 *     tween that is overwriting this one should be re-initted() with the changed properties.
		 */
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------
		 ...
		 * To illustrate the default behavior, the following code will immediately set the <code>alpha</code> of
		 * <code>mc</code>  to 0 and then wait 2 seconds before tweening the <code>alpha</code> back to 1 over the
		 * course  of 1.5 seconds:<br /><br /><code>
		 *
		 ...
		 *     DisplayObject.
		 * @param duration Duration in seconds (or in frames if the tween's timing mode is frames-based)
		 * @param vars An object containing the start values of the properties you're tweening. For example, to tween
		 *     from x=100, y=100, you could pass {x:100, y:100}. It can also contain special properties like
		 ...
		 * @param useFrames If the delay should be measured in frames instead of seconds, set useFrames to true
		 *     (default is false)
		 * @return TweenLite instance
		 */
		 ...
		 * completing them first. If, for example, you want to kill all tweens of the "mc" object, you'd do:<br /><br
		 * /><code>
		 *
		 * TweenLite.killTweensOf(mc);<br /><br /></code>
		 *
		 * You can also just kill certain tweening properties of a particular object. For example, if you only want to
		 * kill all  the "alpha" and "x" portions of mc's tweens, you'd do:<br /><br /><code>
		 *
		 ...
		 * @param vars An object defining which tweening properties should be killed (null causes all properties to be
		 *     killed). For example, if you only want to kill "alpha" and "x" tweens of object "mc", you'd do
		 *     <code>myTimeline.killTweensOf(mc, true, {alpha:true, x:true})</code>. If there are no tweening
		 *     properties remaining in a tween after the indicated properties are killed, the entire tween is killed,
		 *     meaning any onComplete, onUpdate, onStart, etc. won't fire.
		 */

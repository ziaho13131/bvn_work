 ...
 */
	public class TweenCore {
		 ...
		 * @param suppressEvents If true, no events or callbacks will be triggered as the "virtual playhead" moves to
		 *     the new position (onComplete, onUpdate, onReverseComplete, etc. of this tween/timeline and any of its
		 *     child tweens/timelines won't be triggered, nor will any of the associated events be dispatched)
		 */
		 ...
		 * 
		 * @param forceResume If true, it will resume() immediately upon reversing. Otherwise its paused state will
		 *     remain unchanged.
		 */
		 ...
		 * @param suppressEvents If true, no events or callbacks will be triggered for this render (like onComplete,
		 *     onUpdate, onReverseComplete, etc.)
		 * @param force Normally the tween will skip rendering if the time matches the cachedTotalTime (to improve
		 *     performance), but if force is true, it forces a render. This is primarily used internally for tweens
		 *     with durations of zero in TimelineLite/Max instances.
		 */
		 ...
		 * a tween/timeline, it will be re-initialized the next time it renders and its <code>vars</code> object will
		 * be re-parsed.  The timing of the tween/timeline (duration, startTime, delay) will NOT be affected. Another
		 * example would be if you have a <code>TweenMax(mc, 1, {x:100, y:100})</code> that ran when mc.x and mc.y were
		 * initially at 0, but now mc.x  and mc.y are 200 and you want them tween to 100 again, you could simply
		 * <code>invalidate()</code> the tween and
		 * <code>restart()</code> it. Without invalidating first, restarting it would cause the values jump back to 0
		 * immediately
		 * (where they started when the tween originally began). When you invalidate a timeline, it automatically
		 * invalidates  all of its children.
		 **/
		 ...
		 * @param ignoreTimeline By default, the tween/timeline will remove itself from its parent timeline when it is
		 *     disabled, and add itself when it is enabled, but this parameter allows you to override that behavior.
		 * @return Boolean value indicating whether or not important properties may have changed when the TweenCore was
		 *     enabled/disabled. For example, when a motionBlur (plugin) is disabled, it swaps out a BitmapData for the
		 *     target and may alter the alpha. We need to know this in order to determine whether or not a new tween
		 *     that is overwriting this one should be re-initted() with the changed properties.
		 **/
		 ...
		 * the cacheIsDirty property to true forces any necessary recalculation of its cachedDuration and
		 *     cachedTotalDuration  properties and sorts the affected timelines' children TweenCores so that they're in
		 *     the proper order  next time the duration or totalDuration is requested. We don't just recalculate them
...
//---- GETTERS / SETTERS ------------------------------------------------------------
		 ...
		 * or repeatDelays. <code>totalDuration</code>, by contrast, does include repeats and repeatDelays. If you
		 * alter
		 * the <code>duration</code> of a tween while it is in-progress (active), its <code>startTime</code> will
		 * automatically  be adjusted in order to make the transition smoothly (without a sudden skip).
		 **/
		 ...
		 * or repeatDelays (which are only available on TweenMax and TimelineMax). <code>duration</code>, by contrast,
		 * does
		 * <b>NOT</b> include repeats and repeatDelays. So if a TweenMax's <code>duration</code> is 1 and it has a
		 * repeat of 2,  the <code>totalDuration</code> would be 3.
		 **/ 
		 ...
		 * properties over the course of the tween, you'd see <code>currentTime</code> go from 0 to 5 twice (one for
		 * each cycle) in the same time it takes <code>totalTime</code> go from 0 to 10.
		 **/
